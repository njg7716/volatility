...
import volatility.plugins.freebsd.pid_hash_table as pid_hash_table
import volatility.plugins.freebsd.pgrp_hash_table as pgrp_hash_table
import volatility.plugins.freebsd.session_hash_table as session_hash_table
import volatility.plugins.freebsd.pstasks as pstasks
from volatility.renderers import TreeGrid
from volatility.renderers.basic import Address

class freebsd_psxview(common.AbstractFreebsdCommand):
    "Find hidden processes with various process listings"

    def _get_pslist(self):
        return [p.v() for p in pslist.freebsd_pslist(self._config).calculate()]

    def _get_parent_pointers(self):
        return [p.p_pptr.v() for p in pslist.freebsd_pslist(self._config).calculate()]

    def _get_pid_hash_table(self):
        return [p.v() for p in pid_hash_table.freebsd_pid_hash_table(self._config).calculate()]
    
    def _get_pgrp_hash_table(self):
        return [p.v() for p in pgrp_hash_table.freebsd_pgrp_hash_table(self._config).calculate()]
 
    def _get_session_hash_table(self):
        return [s.s_leader.v() for s in session_hash_table.freebsd_list_sessions(self._config).calculate() if s.s_leader.is_valid()]
    
    def _get_procs_from_tasks(self):
        return [p.v() for p in pstasks.freebsd_pstasks(self._config).calculate()]       
